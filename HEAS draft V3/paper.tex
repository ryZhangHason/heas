...
Optimization libraries (DEAP~\cite{fortin2012deap}, Optuna~\cite{akiba2019optuna})
provide flexible search backends but require researchers to re-implement metric
extraction for each project. HEAS targets the integration gap directly: the
uniform \texttt{metrics\_episode()} contract is shared identically by the
evolutionary algorithm, tournament scorer, and bootstrap CI pipeline, making
metric divergence structurally impossible regardless of how many objectives or
scenarios are added.
...
\textbf{Scope of the comparison.} We acknowledge that any framework bundling
EA, tournament, and CI infrastructure will show a similar LOC reduction.
HEAS's distinctive contribution is \emph{structural enforcement}: the metric
contract (C2) makes silent divergence architecturally impossible, not just
discouraged by convention. A thin utility wrapper around Mesa would still
allow the EA, tournament, and CI to consume different views of the same
metric. Additionally, the 160-line cost is not a one-time project setup: each
new Mesa study reimplements it from scratch, because Mesa provides no standard
utility layer for seed management, parallel evaluation, or bootstrap CI. The
HEAS equivalent is zero LOC because these concerns are resolved at the
framework level.
...
episode budgets (4, 10, 25, 50, 100 per scenario), confirming the champion is
well-separated from the field.
...
In the ecological domain (steps\,=\,1,000), NSGA-II achieves mean
HV\,=\,15.46 (std\,8.57, CI\,[12.03,\,18.88]); the bimodal convergence
pattern observed at small scale persists, confirming it is a structural
feature of the ecological landscape rather than a sampling artifact. In the
enterprise domain (steps\,=\,50, 4-gene policy), NSGA-II achieves mean
HV\,=\,5.565\,$\times\!10^6$ (std\,69,624; CI narrow at $\pm\,1.2\%$). The
enterprise Pareto front is unimodal---high-subsidy, moderate-audit-intensity
policies dominate across scenarios---producing stable optimization outcomes
with low between-run variance. The narrow CI ($\pm$1.2\%) contrasts sharply
with the ecological case ($\pm$26\%), illustrating how domain structure shapes
reproducibility independently of framework configuration. In both cases the
framework required no architectural changes: the Arena composition and metric
contract remain identical, and only the layer implementations differ,
validating C1 (Layer/Stream composability) and C2 (uniform metric contract)
