...
criteria. In HEAS the contract prevents this class of accidental divergence
across consumers: all three components read the same \texttt{metrics\_episode()}
key, so they cannot silently diverge. A researcher who implements
\texttt{metrics\_episode()} incorrectly will observe consistent errors rather
than hidden inconsistency between components.

\textbf{Scope of the comparison.} A fairer baseline is \emph{Mesa\,+\,utility
library}: a reusable lab-internal layer would reduce LOC similarly. HEAS's
claim over this baseline is narrower but concrete: C2 enforces the shared
metric interface at the \emph{framework} level, so new projects inherit it
without importing a utility module, and the contract survives collaborator
handoffs. The 97\% number reflects a competent \emph{first-project} Mesa
implementation before any utility layer exists. HEAS's value is highest for
new labs or cross-institution collaborations where such a library has not yet
been developed.
...
episode budgets (4, 10, 25, 50, 100 per scenario). This unanimity confirms
genuine dominance rather than single-scenario margin; in closer contests the
rules diverge and reveal whether the competitive structure is scenario-specific
or magnitude-driven (see Section~3.4 for rule selection guidance).
...
remain identical, validating C1 and C2 at operational scale.
...
implementation will be consistently wrong rather than silently inconsistent.
